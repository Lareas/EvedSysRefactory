 ...
 * @author luiza
 */
public class PlacarMatriController implements Initializable {
    ...
    private String sqlMatric
            = "SELECT dcg.DadoCadastroGeralId, dcg.matricula, dcg.nome	"
            + " FROM cadastroalunodisciplina pup  "
    ...
    public String sqlRelat = " WHERE cad.CodGrade = $P(CODGRADE) "; // pega do WHERE para frente
    ...
    public void initialize(URL url, ResourceBundle rb) {
        cbListagens.setItems(getListagens(3)); // 1 =  listagens lista de presença; 2 = placar; 3 = matric
...
//        tfNumber.setStyle("-fx-text-fill: white;"); //The background of the cell in yellow
        ...
        try {
            ...
            sqlRelat = sqlRelat + " GROUP BY dcg.DadoCadastroGeralId ORDER BY dcg.nome ";
            ...
            Logger.getLogger(MenuPrincipalController.class
                    .getName()).log(Level.SEVERE, null, ex);
            mostraMsg("Erro 018k - em aluMatri()", "" + ex, 2);
        }
        ...
        if (cbListagens.getSelectionModel().getSelectedItem() != null) {
            ...
            try {
               ...
               // sqlRelat = sqlRelat + " GROUP BY dcg.DadoCadastroGeralId ORDER BY dcg.nome ";
                ...
                if (cbListagens.getSelectionModel().getSelectedItem().equals("Listagem alunos matriculados - cód., matr., aluno")) { // listagem placar
                    parametros.put("RESTANTE_COND", sqlRelat);
                    parametros.put("TITULO", pi.getSemestre() + " - " + lblTitPlacar.getText() + " (" + lblQtd.getText() + " registros)");
