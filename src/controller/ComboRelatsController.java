 ...
 * @author ticoa
 */
public class ComboRelatsController implements Initializable {
    ...
    @FXML
    private JFXTextField edDescCateg1;
    @FXML
    private TableColumn<?, ?> tc31;
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Categoria nº " + reg_atual.getCategoriaAlunoId());
...
//    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getCategoriaAlunoId());
