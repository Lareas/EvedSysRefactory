 ...
 * @author ticoa
 */
public class OutrosDocsController implements Initializable {
    ...
    @FXML
    private TableView<Docsoficiais> tvPesquisa;
    @FXML
    private TableColumn<Docsoficiais, Integer> tc1;
    @FXML
    private TableColumn<Docsoficiais, String> tc2;
    @FXML
    ...
    // VARIÁVEIS LOCAIS
    // --------------------
    private ObservableList<Docsoficiais> dados;
    private DocsoficiaisJpaController jpaCon;
    Docsoficiais reg_atual = new Docsoficiais();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new DocsoficiaisJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
        ...
        cbIndice.getItems().addAll("Código", "Docsoficiais");
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("docOficialId"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("nomeDocOficial"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
    }

    public ObservableList<Docsoficiais> getMeusDados() {

        jpaCon = new DocsoficiaisJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDocsoficiaisEntities());
        txtPesq.setText("");

        int rowcount = jpaCon.getDocsoficiaisCount();
        if (rowcount > 1) {
    ...
    public ObservableList<Docsoficiais> getMinhaPesqDados(String pesq) {

        jpaCon = new DocsoficiaisJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getOutDocPesq(pesq));
        lblRegistros.setText("");

        Long rowcount = getQuantosNomes("Docsoficiais", "turno", pesq);
    ...
    public ObservableList<Docsoficiais> getPesquisa() {
        jpaCon = new DocsoficiaisJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDocsoficiaisEntities());
        txtPesq.setText("");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Docsoficiais> não pode ficar vazio.\n";
            cte++;
        }
        ...
        if (cte > 0) {
            ...
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Docsoficiais>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                Docsoficiais obj = new Docsoficiais();
                                obj.setNomeDocOficial(edNomeProg.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Formação nº " + reg_atual.getDocOficialId());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Documento nº " + reg_atual.getDocOficialId());
        edNomeProg.setText(reg_atual.getNomeDocOficial());
    }

    public void preencheObjComEds() {

        reg_atual.setNomeDocOficial(edNomeProg.getText());
    }
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new DocsoficiaisJpaController();
        List lis = jpaCon.getOutDocPesqExac(pesq);

        Docsoficiais prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Docsoficiais) lis.get(i);
                if (!prog.getDocOficialId().equals(reg_atual.getDocOficialId())) {
                    podegravar = false;
                }
            }
        } else {
