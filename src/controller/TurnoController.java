...
public class TurnoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbIndice.getItems().addAll("Código", "Turno");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Turno> não pode ficar vazio.\n";
            cte++;
        }
        ...
        if (cte > 0) {
            ...
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Turno>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Turno nº " + reg_atual.getTurnoId());
                                inserindo = false;
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getTurnoId());
