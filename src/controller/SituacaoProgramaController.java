 ...
 * @author ticoa
 */
public class SituacaoProgramaController implements Initializable {
    ...
    @FXML
    private TableView<Dadocadastroprogramasituacao> tvPesquisa;
    @FXML
    private TableColumn<Dadocadastroprogramasituacao, Integer> tc1;
    @FXML
    private TableColumn<Dadocadastroprogramasituacao, String> tc2;
    @FXML
    ...
    // VARIÁVEIS LOCAIS
    // --------------------
    private ObservableList<Dadocadastroprogramasituacao> dados;
    private DadocadastroprogramasituacaoJpaController jpaCon;
    Dadocadastroprogramasituacao reg_atual = new Dadocadastroprogramasituacao();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new DadocadastroprogramasituacaoJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
        ...
        cbIndice.getItems().addAll("Código", "Descrição");
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("dadoCadastroProgramaSituacaoId"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("dadoCadastroProgramaSituacao"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
        lblRegistros.setText("Registros exibidos: " + tvPesquisa.itemsProperty().get().size());
    }

    public ObservableList<Dadocadastroprogramasituacao> getMeusDados() {

        jpaCon = new DadocadastroprogramasituacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDadocadastroprogramasituacaoEntities());
        txtPesq.setText("");
    ...
    public ObservableList<Dadocadastroprogramasituacao> getMinhaPesqDados(String pesq) {

        jpaCon = new DadocadastroprogramasituacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getDadocadastroprogramasituacaoPesq(pesq));
    ...
    public ObservableList<Dadocadastroprogramasituacao> getPesquisa() {
        jpaCon = new DadocadastroprogramasituacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDadocadastroprogramasituacaoEntities());
        txtPesq.setText("");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Nome do Cadaludissit> não pode ficar vazio.\n";
            cte++;
        }

        // se for inserção, verificar se Nome do Cadaludissit já existe
        if (cte > 0) {
            // há erros
            ...
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Situação do aluno>.", 2);
            } else {
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                Dadocadastroprogramasituacao obj = new Dadocadastroprogramasituacao();
                                obj.setDadoCadastroProgramaSituacao(edNomeProg.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Situação nº " + reg_atual.getDadoCadastroProgramaSituacao());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Situação nº " + reg_atual.getDadoCadastroProgramaSituacaoId());
        edNomeProg.setText(reg_atual.getDadoCadastroProgramaSituacao());
    }

    public void preencheObjComEds() {
        reg_atual.setDadoCadastroProgramaSituacao(edNomeProg.getText());
    }

    public void limpaCampos() {
        txtCodPK.setText("Situação nº ");
        edNomeProg.setText("");
    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getDadoCadastroProgramaSituacaoId());
            alert.setContentText("Deseja realmente apagar esse registro?\nOs dados serão perdidos!");
            Optional<ButtonType> option = alert.showAndWait();

            if (option.get() == ButtonType.OK) {
                try {
                    jpaCon.destroy(reg_atual.getDadoCadastroProgramaSituacaoId());
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new DadocadastroprogramasituacaoJpaController();
        List lis = jpaCon.getDadocadastroprogramasituacaoPesqExac(pesq);

        Dadocadastroprogramasituacao prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Dadocadastroprogramasituacao) lis.get(i);
                if (!prog.getDadoCadastroProgramaSituacaoId().equals(reg_atual.getDadoCadastroProgramaSituacaoId())) {
                    podegravar = false;
                }
            }
        } else {
