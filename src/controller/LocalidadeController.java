 ...
 * @author ticoa
 */
public class LocalidadeController implements Initializable {
    ...
    @FXML
    private TableView<Localidade> tvPesquisa;
    @FXML
    private TableColumn<Localidade, Integer> tc1;
    @FXML
    private TableColumn<Localidade, String> tc2;
    @FXML
    ...
    private LocalidadeJpaController jpaCon;
    Localidade reg_atual = new Localidade();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new LocalidadeJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
        ...
        cbIndice.getItems().addAll("Código", "Localidade");
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("localidadeId"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("localidade"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
    }

    public ObservableList<Localidade> getMeusDados() {

        jpaCon = new LocalidadeJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findLocalidadeEntities());
        txtPesq.setText("");

        int rowcount = jpaCon.getLocalidadeCount();
        if (rowcount > 1) {
    ...
    public ObservableList<Localidade> getMinhaPesqDados(String pesq) {
        
        jpaCon = new LocalidadeJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getLocalidadePesq(pesq));
        lblRegistros.setText("");

        Long rowcount = getQuantosNomes("Localidade", "localidade", pesq);
    ...
    public ObservableList<Localidade> getPesquisa() {
        jpaCon = new LocalidadeJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findLocalidadeEntities());
        txtPesq.setText("");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Nome do Localidade> não pode ficar vazio.\n";
            cte++;
        }

        // se for inserção, verificar se Nome do Localidade já existe
        if (cte > 0) {
            // há erros
            mostraMsg("Há erro(s) de preenchimento.", warn, 2);
        } else {
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Localidade>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                obj.setLocalidade(edNomeProg.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Localidade nº " + reg_atual.getLocalidadeId());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Localidade nº " + reg_atual.getLocalidadeId());
        edNomeProg.setText(reg_atual.getLocalidade());
    }

    public void preencheObjComEds() {
        reg_atual.setLocalidade(edNomeProg.getText());
    }

    public void limpaCampos() {
        txtCodPK.setText("Localidade nº ");
        edNomeProg.setText("");
    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getLocalidadeId());
            alert.setContentText("Deseja realmente apagar esse registro?\nOs dados serão perdidos!");
            Optional<ButtonType> option = alert.showAndWait();

            if (option.get() == ButtonType.OK) {
                try {
                    jpaCon.destroy(reg_atual.getLocalidadeId());
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new LocalidadeJpaController();
        List lis = jpaCon.getLocalidadeExac(pesq);

        Localidade prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Localidade) lis.get(i);
                if (!prog.getLocalidadeId().equals(reg_atual.getLocalidadeId())) {
                    podegravar = false;
                }
            }
        } else {
