...
public class GradeMContoller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbIndice.getItems().addAll("Código", "Ano letivo");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Ano Letivo> não pode ficar vazio.\n";
            cte++;
        }

        if (edDescCateg.getText().equals("")) {
            warn = warn + "O campo <Semestre> não pode ficar vazio.\n";
            cte++;
        }
        ...
        if (cte > 0) {
            ...
            if (verifPodeGravar(edNomeProg.getText(), edDescCateg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmos Ano Letivo e Semestre.", 2);
            } else {
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Grade nº " + reg_atual.getCodGrade());
...
//    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getCodGrade());
