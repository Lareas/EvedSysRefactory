...
public class SiglaestadoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        //////////////////////////////////////////
        //  INICIALIZA AMBIENTE E VARIÁVEIS
        //////////////////////////////////////////
        ...
        cbIndice.getItems().addAll("Código", "Sigla UF", "UF");
...
//        Boolean emesmo = isInt(txtPesq,txtPesq.getText());
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Sigla UF> não pode ficar vazio.\n";
            cte++;
        }

        if (edUF.getText().equals("")) {
            warn = warn + "O campo <UF> não pode ficar vazio.\n";
            cte++;
        }
        

        // se for inserção, verificar se Nome do Siglaestado já existe
        if (cte > 0) {
            // há erros
            mostraMsg("Há erro(s) de preenchimento.", warn, 2);
        } else {
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Sigla UF>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Sigla UF nº " + reg_atual.getSiglaEstadoId());
                                inserindo = false;
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getSiglaEstadoId());
