 ...
 * @author ticoa
 */
public class CadaludissitController implements Initializable {
    ...
    @FXML
    private TableView<Cadaludissit> tvPesquisa;
    @FXML
    private TableColumn<Cadaludissit, Integer> tc1;
    @FXML
    private TableColumn<Cadaludissit, String> tc2;
    @FXML
    private TableColumn<Cadaludissit, String> tc3;
    @FXML
    private TableColumn<Cadaludissit, String> tc4;
    @FXML
    private TableColumn<Cadaludissit, String> tc5;
    @FXML
    ...
    // VARIÁVEIS LOCAIS
    // --------------------
    private ObservableList<Cadaludissit> dados;
    private CadaludissitJpaController jpaCon;
    Cadaludissit reg_atual = new Cadaludissit();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new CadaludissitJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("cadsId"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("descricao"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
    }

    public ObservableList<Cadaludissit> getMeusDados() {

        jpaCon = new CadaludissitJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findCadaludissitEntities());
        txtPesq.setText("");

        int rowcount = jpaCon.getCadaludissitCount();
        if (rowcount > 1) {
    ...
    public ObservableList<Cadaludissit> getMinhaPesqDados(String pesq) {

        jpaCon = new CadaludissitJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getCadaludissitPesq(pesq));
        lblRegistros.setText("");

        Long rowcount = getQuantosNomes("Cadaludissit", "descricao", pesq);
    ...
    public ObservableList<Cadaludissit> getPesquisa() {
        jpaCon = new CadaludissitJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findCadaludissitEntities());
        txtPesq.setText("");
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                Cadaludissit obj = new Cadaludissit();
                                obj.setDescricao(edNomeProg.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Situação nº " + reg_atual.getCadsId());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Situação nº " + reg_atual.getCadsId());
        edNomeProg.setText(reg_atual.getDescricao());
    }

    public void preencheObjComEds() {
        reg_atual.setDescricao(edNomeProg.getText());
    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getCadsId());
            alert.setContentText("Deseja realmente apagar esse registro?\nOs dados serão perdidos!");
            Optional<ButtonType> option = alert.showAndWait();

            if (option.get() == ButtonType.OK) {
                try {
                    jpaCon.destroy(reg_atual.getCadsId());
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new CadaludissitJpaController();
        List lis = jpaCon.getCadaludissitPesqExac(pesq);

        Cadaludissit prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Cadaludissit) lis.get(i);
                if (!prog.getCadsId().equals(reg_atual.getCadsId())) {
                    podegravar = false;
                }
            }
        } else {
