 ...
 * @author ticoa
 */
public class DenominacaoController implements Initializable {
    ...
    @FXML
    private TableView<Denominacao> tvPesquisa;
    @FXML
    private TableColumn<Denominacao, Integer> tc1;
    @FXML
    private TableColumn<Denominacao, String> tc2;
    @FXML
    private TableColumn<Denominacao, String> tc3;
    @FXML
    ...
    // VARIÁVEIS LOCAIS
    // --------------------
    private ObservableList<Denominacao> dados;
    private DenominacaoJpaController jpaCon;
    Denominacao reg_atual = new Denominacao();
    private boolean inserindo, editando;

    @FXML
    private JFXTextField edNomeProg;
    @FXML
    private JFXTextField edDescNome;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new DenominacaoJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
        ...
        cbIndice.getItems().addAll("Código", "Denominação", "Ramificação");
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("denominacaoId"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("denominacao"));
        tc3.setCellValueFactory(new PropertyValueFactory<>("Ramificacao"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
    }

    public ObservableList<Denominacao> getMeusDados() {

        jpaCon = new DenominacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDenominacaoEntities());
        txtPesq.setText("");

        int rowcount = jpaCon.getDenominacaoCount();
        if (rowcount > 1) {
    ...
    public ObservableList<Denominacao> getMinhaPesqDados(String pesq) {

        jpaCon = new DenominacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getDenominacaoPesq(pesq));
        //  txtPesq.setText("");
        lblRegistros.setText("");

        Long rowcount = getQuantosNomes("Denominacao", "denominacao", pesq);
    ...
    public ObservableList<Denominacao> getPesquisa() {
        jpaCon = new DenominacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findDenominacaoEntities());
        txtPesq.setText("");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Denominacao> não pode ficar vazio.\n";
            cte++;
        }

        if (edDescNome.getText().equals("")) {
        ...
        if (cte > 0) {
            ...
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Denominacao>.", 2);
            } else {
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                Denominacao obj = new Denominacao();
                                obj.setDenominacao(edNomeProg.getText());
                                obj.setRamificacao(edDescNome.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Denominação nº " + reg_atual.getDenominacaoId());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Denominação nº " + reg_atual.getDenominacaoId());
        edNomeProg.setText(reg_atual.getDenominacao());
        edDescNome.setText(reg_atual.getRamificacao());
    }

    public void preencheObjComEds() {

        reg_atual.setDenominacao(edNomeProg.getText());
        reg_atual.setRamificacao(edDescNome.getText());
    }

    public void limpaCampos() {
        txtCodPK.setText("Denominação nº ");
        edNomeProg.setText("");
        edDescNome.setText("");
    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getDenominacaoId());
            alert.setContentText("Deseja realmente apagar esse registro?\nOs dados serão perdidos!");
            Optional<ButtonType> option = alert.showAndWait();

            if (option.get() == ButtonType.OK) {
                try {
                    jpaCon.destroy(reg_atual.getDenominacaoId());
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new DenominacaoJpaController();
        List lis = jpaCon.getDenominacaoPesqExac(pesq);

        Denominacao prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Denominacao) lis.get(i);
                if (!prog.getDenominacaoId().equals(reg_atual.getClass())) {
                    podegravar = false;
                }
            }
        } else {
