...
public class SexoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbIndice.getItems().addAll("Código", "Sexo", "Sigla");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Sexo> não pode ficar vazio.\n";
            cte++;
        }
        
        // CONSISTÊNCIA DOS CAMPOS
        if (edSigla.getText().equals("")) {
            warn = warn + "O campo <Sigla> não pode ficar vazio.\n";
            cte++;
        }

        // se for inserção, verificar se Nome do Sexo já existe
        if (cte > 0) {
            // há erros
            mostraMsg("Há erro(s) de preenchimento.", warn, 2);
        } else {
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Sexo>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Sexo  nº " + reg_atual.getSexoId());
                                inserindo = false;
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getSexoId());
