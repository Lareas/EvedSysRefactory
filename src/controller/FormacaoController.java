 ...
 * @author ticoa
 */
public class FormacaoController implements Initializable {
    ...
    @FXML
    private TableView<Formacao> tvPesquisa;
    @FXML
    private TableColumn<Formacao, Integer> tc1;
    @FXML
    private TableColumn<Formacao, String> tc2;
    @FXML
    ...
    // VARIÁVEIS LOCAIS
    // --------------------
    private ObservableList<Formacao> dados;
    private FormacaoJpaController jpaCon;
    Formacao reg_atual = new Formacao();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        jpaCon = new FormacaoJpaController(); // COLOQUEI UM CONSTRUCTOR VAZIO
        ...
        cbIndice.getItems().addAll("Código", "Formacao");
    ...
    public void initColumns() {
        tc1.setCellValueFactory(new PropertyValueFactory<>("codFormacao"));
        tc2.setCellValueFactory(new PropertyValueFactory<>("nomeFormacao"));
        tvPesquisa.setItems(getMeusDados());
        tvPesquisa.refresh();
    }

    public ObservableList<Formacao> getMeusDados() {

        jpaCon = new FormacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findFormacaoEntities());
        txtPesq.setText("");

        int rowcount = jpaCon.getFormacaoCount();
        if (rowcount > 1) {
    ...
    public ObservableList<Formacao> getMinhaPesqDados(String pesq) {

        jpaCon = new FormacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.getFormacaoPesq(pesq));
        lblRegistros.setText("");

        Long rowcount = getQuantosNomes("Formacao", "turno", pesq);
    ...
    public ObservableList<Formacao> getPesquisa() {
        jpaCon = new FormacaoJpaController();
        dados = FXCollections.observableArrayList(jpaCon.findFormacaoEntities());
        txtPesq.setText("");
    ...
    private void clicouConfirma(ActionEvent event) {
        ...
        if (edNomeProg.getText().equals("")) {
            warn = warn + "O campo <Formacao> não pode ficar vazio.\n";
            cte++;
        }
        ...
        if (cte > 0) {
            ...
            if (verifPodeGravar(edNomeProg.getText()) == false) {
                mostraMsg("Não é possível gravar!", "Já existe outro registro com o mesmo campo <Formacao>.", 2);
            } else {
                Alert alert = new Alert(AlertType.CONFIRMATION);
                    ...
                    if (editando == true) {
                        ...
                        if (inserindo == true) {
                            try {
                                ...
                                obj.setNomeFormacao(edNomeProg.getText());

                                jpaCon.create(obj);
                                reg_atual = obj;
                                txtCodPK.setText("Formação nº " + reg_atual.getCodFormacao());
                                inserindo = false;
    ...
    public void preencheEdsComObj() {
        txtCodPK.setText("Formacao nº " + reg_atual.getCodFormacao());
        edNomeProg.setText(reg_atual.getNomeFormacao());
    }

    public void preencheObjComEds() {

        reg_atual.setNomeFormacao(edNomeProg.getText());
    }

    public void limpaCampos() {
        txtCodPK.setText("Formacao nº ");
        edNomeProg.setText("");
    }
    ...
    private void clicouApaga(ActionEvent event) {
        if (!edNomeProg.getText().equals("")) {
            Alert alert = new Alert(AlertType.CONFIRMATION);
            alert.setHeaderText("Confirmação - apagar registro " + reg_atual.getCodFormacao());
            alert.setContentText("Deseja realmente apagar esse registro?\nOs dados serão perdidos!");
            Optional<ButtonType> option = alert.showAndWait();

            if (option.get() == ButtonType.OK) {
                try {
                    jpaCon.destroy(reg_atual.getCodFormacao());
...
//        cbIndice.getSelectionModel().getSelectedIndex();
    ...
    private Boolean verifPodeGravar(String pesq) {
        ...
        Long achados = 0L;
        jpaCon = new FormacaoJpaController();
        List lis = jpaCon.getFormacaoPesqExac(pesq);

        Formacao prog;
        if (editando == true) {

            for (int i = 0; i < lis.size(); i++) {
                prog = (Formacao) lis.get(i);
                if (!prog.getCodFormacao().equals(reg_atual.getCodFormacao())) {
                    podegravar = false;
                }
            }
        } else {
